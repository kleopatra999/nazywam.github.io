...
ApplicationMain.preloader_onComplete = function(event) {
	...
	if(hasMain) Reflect.callMethod(Main,Reflect.field(Main,"main"),[]); else {
		...
		if(js.Boot.__instanceof(instance,openfl.display.DisplayObject)) openfl.Lib.current.addChild(instance); else {
			haxe.Log.trace("Error: No entry point found",{ fileName : "ApplicationMain.hx", lineNumber : 928, className : "ApplicationMain", methodName : "preloader_onComplete"});
			haxe.Log.trace("If you are using DCE with a static main, you may need to @:keep the function",{ fileName : "ApplicationMain.hx", lineNumber : 929, className : "ApplicationMain", methodName : "preloader_onComplete"});
		}
	}
};
...
Main.__name__ = ["Main"];
Main.main = function() {
	openfl.Lib.current.addChild(new Board());
};
...
var Board = function() {
	...
	openfl.Lib.current.stage.set_scaleX(openfl.Lib.current.stage.stageWidth / 1950);
	openfl.Lib.current.stage.set_scaleY(openfl.Lib.current.stage.get_scaleX());
	...
	while(_g < 4) {
		...
		while(_g1 < 15) {
			...
			var num = "";
			if(n <= 10) if(n == null) num = "null"; else num = "" + n; else switch(n) {
			case 11:
				num = "J";
				break;
			case 12:
			...
			var c = new Card(type,num);
			c.set_x((n - 2) * 150);
			c.set_y(t * 200);
			this.cards.push(c);
			this.addChild(c);
			c.addEventListener(openfl.events.MouseEvent.CLICK,$bind(c,c.click));
		}
	}
};
$hxClasses["Board"] = Board;
Board.__name__ = ["Board"];
Board.__super__ = openfl.display.Sprite;
Board.prototype = $extend(openfl.display.Sprite.prototype,{
	__class__: Board
});
var Card = function(type,num) {
	this.turned = false;
	openfl.display.Sprite.call(this);
	this.type = type;
	this.num = num;
	this.card = new openfl.display.Bitmap(openfl.Assets.getBitmapData("assets/card" + type + num + ".png"));
	this.addChild(this.card);
	this.cover = new openfl.display.Bitmap(openfl.Assets.getBitmapData("assets/cardBack_red5.png"));
	this.addChild(this.cover);
	this.cover.set_visible(false);
};
$hxClasses["Card"] = Card;
Card.__name__ = ["Card"];
Card.__super__ = openfl.display.Sprite;
Card.prototype = $extend(openfl.display.Sprite.prototype,{
	click: function(e) {
		this.turned = !this.turned;
		this.card.set_visible(!this.turned);
		this.cover.set_visible(this.turned);
	}
	,__class__: Card
});
