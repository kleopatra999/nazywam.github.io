...
Main.main = function() {
	...
	stage.addEventListener(flash.events.Event.ENTER_FRAME,$bind(board,board.tick));
	stage.addEventListener(flash.events.KeyboardEvent.KEY_UP,$bind(board,board.onKeyUp));
	stage.addEventListener(flash.events.KeyboardEvent.KEY_DOWN,$bind(board,board.onKeyDown));
}
...
var Board = function() {
	...
	this.coin.moveToRandomPos(this.maxX,this.maxY);
};
$hxClasses["Board"] = Board;
Board.__name__ = ["Board"];
Board.__super__ = flash.display.Sprite;
Board.prototype = $extend(flash.display.Sprite.prototype,{
	onKeyUp: function(e) {
		switch(e.keyCode) {
		case 32:
			break;
	...
	,drawGoldLayer: function() {
		this.get_graphics().clear();
		this.get_graphics().beginFill(16174592);
		this.get_graphics().moveTo(0,this.maxY);
		this.get_graphics().lineTo(this.maxX,this.maxY);
		this.get_graphics().lineTo(this.maxX,this.maxY + this.layerHeight);
		this.get_graphics().lineTo(0,this.maxY + this.layerHeight);
		this.get_graphics().lineTo(0,this.maxY);
	...
	,tick: function(_) {
		this.player.tick(this.maxX,this.maxY);
		if(this.coin.intersectsWithPlayer(this.player.posX,this.player.posY,this.player.size)) {
			this.score++;
			this.layerHeight += 10;
			this.maxY -= 10;
			if(this.maxY == 0) {
				console.log("Congratulations, after achieving you lifetime goal you died...");
	...
	,draw: function() {
		this.get_graphics().beginFill(2697513);
		this.get_graphics().moveTo(0,0);
		this.get_graphics().lineTo(this.maxX,0);
		this.get_graphics().lineTo(this.maxX,this.maxY);
		this.get_graphics().lineTo(0,this.maxY);
...
$hxClasses["Coin"] = Coin;
Coin.__name__ = ["Coin"];
Coin.__super__ = flash.display.Sprite;
Coin.prototype = $extend(flash.display.Sprite.prototype,{
	intersectsWithPlayer: function(playerX,playerY,pSize) {
		return this.posX < playerX + pSize && this.posX + this.size > playerX && this.posY < playerY + pSize && this.posY + this.size > playerY;
	}
	,moveToRandomPos: function(maxX,maxY) {
		this.posX = Math.random() * (maxX - this.size);
		this.posY = maxY - (Math.random() * 70 + 50);
	...
	,draw: function() {
		this.get_graphics().clear();
		this.get_graphics().beginFill(16174592);
		this.get_graphics().moveTo(this.posX,this.posY);
		this.get_graphics().lineTo(this.posX + this.size,this.posY);
		this.get_graphics().lineTo(this.posX + this.size,this.posY + this.size);
		this.get_graphics().lineTo(this.posX,this.posY + this.size);
		this.get_graphics().lineTo(this.posX,this.posY);
		this.get_graphics().endFill();
	}
	,__class__: Coin
});
...
Player.__super__ = flash.display.Sprite;
Player.prototype = $extend(flash.display.Sprite.prototype,{
	tick: function(maxX,maxY) {
		this.speedY += 0.1;
		if(this.checkVerticalCollision(maxY)) {
			this.onGround = false;
		...
		if(this.checkHorizontalCollision(maxX)) this.posX += this.speedX; else this.speedX = 0;
		this.slowDown();
		this.set_x(this.posX);
		this.set_y(this.posY);
	}
	,slowDown: function() {
		this.speedX = (this.speedX * 15 + this.finalSpeedX) / 16;
	}
	,checkHorizontalCollision: function(maxX) {
		return this.posX + this.speedX >= 0 && this.posX + this.size + this.speedX <= maxX;
	}
	,checkVerticalCollision: function(maxY) {
		return this.posY + this.speedY >= 0 && this.posY + this.size + this.speedY <= maxY;
	}
	,move: function() {
		this.finalSpeedX = (this.left + this.right) * 3;
	}
	,jump: function() {
		if(this.onGround) {
			this.onGround = false;
	...
	,draw: function() {
		this.get_graphics().clear();
		this.get_graphics().beginFill(14474460);
		this.get_graphics().moveTo(this.posX,this.posY);
		this.get_graphics().lineTo(this.posX + this.size,this.posY);
		this.get_graphics().lineTo(this.posX + this.size,this.posY + this.size);
		this.get_graphics().lineTo(this.posX,this.posY + this.size);
